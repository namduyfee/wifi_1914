...
void FIREWORK_Test_FullLED() {
    ...
    /* Đổi màu */
    if((currentTick - changeColor) >= 2000) {
        changeColor = currentTick;
        currentColorID = (currentColorID + 1) & 0x3;
    }
    switch(currentColorID) 
    {
        ...
        case 3 : {      // White
            currentColor = COLOR_WHITE_TEST;
            break;
        }
    }
    /* Refresh LED */
    if((currentTick - reload_led) >= 100) 
    {
...
void FIREWORK_Test_nCot() 
{
    ...
    if((user_millis() - reload_led) >= 100) 
    {
        ...
        for(uint16_t idx_led = 0; idx_led < MAX_CHIP_SERI; idx_led++) 
        {
            ...
            {
                DataLeds[idx_led] = COLOR_WHITE_TEST;
            }
            else 
            {
                DataLeds[idx_led] = COLOR_BLACK;            
            }
        }
        /* Đẩy ra LED */
        FIREWORK_DisplayData(DataLeds, MAX_CHIP_SERI);
...
void FIREWORK_Test_nTia() {
    ...
    if((user_millis() - reload_led) >= 100) 
    {
        ...
        for(uint16_t idx_led = 0; idx_led < MAX_CHIP_SERI; idx_led++) 
        {
            ...
            else if(idx_led < DataSaveRam.SizeCot + DataSaveRam.SizeTia) 
            {
                DataLeds[idx_led] = COLOR_WHITE_TEST;
            }
            else 
    ...
    if((user_millis() - reload_led) >= (FIREWORK_MAX_SPD_COT - DataSaveRam.SpeedCot)) 
    {
        ...
        for(int16_t idx_led = 0; idx_led < Meteor_Length; idx_led++) 
        {
            if(((pos_led + idx_led) >= 0) && ((pos_led + idx_led) < DataSaveRam.SizeCot)) 
            {
                ...
                DataLeds[pos_led + idx_led] = (Color_RGB_t){color_temp, color_temp, color_temp};
            }
        }
        for(int16_t idx_led = DataSaveRam.SizeCot; idx_led < MAX_CHIP_SERI; idx_led++) 
        {
            DataLeds[idx_led] = COLOR_BLACK;
        }
        /* Đẩy ra LED */
    ...
    if((user_millis() - reload_led) >= (FIREWORK_MAX_SPD_TIA - DataSaveRam.SpeedTia)) {
        ...
        for(uint8_t idx_led = 0; idx_led < Meteor_Length; idx_led++) {
            if(((pos_led + idx_led) >= 0) && ((pos_led + idx_led) < DataSaveRam.SizeTia)) {
                ...
                DataLeds[DataSaveRam.SizeCot + pos_led + idx_led] = (Color_RGB_t){color_temp, color_temp, color_temp};
            }
        }
        for(int16_t idx_led = 0; idx_led < DataSaveRam.SizeCot; idx_led++) 
        {
            DataLeds[idx_led] = COLOR_BLACK;
        }
        /* Đẩy ra LED */
